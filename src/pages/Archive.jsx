...
export default function Archive({ onAuthRequired }) {
  const { user } = useAuth()
  const [tab, setTab] = useState('recommended') // 'recommended' | 'reacted'
  ...
  async function loadArchive() {
    ...
    try {
      ...
      const { data: myReactions } = await supabase
        .from('reactions')
        .select(`color, score_value, created_at, posts (*, profiles:user_id (id, username, total_score), reactions (color, user_id, score_value))`)
        .eq('user_id', user.id)
        .order('created_at', { ascending: false })

      const enriched = (myReactions || []).map(r => {
        const post = r.posts
        if (!post) return null
        const counts = {}
        post.reactions?.forEach(rx => { counts[rx.color] = (counts[rx.color] || 0) + 1 })
        return {
          ...post,
          reaction_counts: counts,
          user_reaction: r.color,
          my_reaction: r.color,
          reacted_at: r.created_at,
        }
      }).filter(Boolean)

      setReactedPosts(enriched)
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <button
          ...
          className="bg-groove-vinyl text-groove-cream font-mono text-xs tracking-widest uppercase px-6 py-3 rounded hover:bg-groove-brown transition-colors"
        >
          Sign In
        </button>
      </div>
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {tabs.map(t => (
          <button
            ...
            className={`flex-1 font-mono text-xs tracking-wider uppercase py-2 rounded transition-all ${
              tab === t.key
                ? 'bg-groove-vinyl text-groove-cream shadow-sm'
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => <div key={i} className="h-32 bg-groove-paper rounded-lg animate-pulse" />)}
        </div>
      ) : tab === 'recommended' ? (
        myPosts.length === 0 ? (
          <div className="text-center py-12">
            <p className="font-display font-bold text-groove-vinyl text-xl">You haven't dropped any picks yet.</p>
            <p className="font-mono text-xs text-groove-label mt-2">Head to the feed to share your first recommendation.</p>
          </div>
        ) : (
          <div>
            {/* Reaction summary across all posts */}
                ...
                {REACTIONS.map(r => {
                  ...
                  return (
                    <div key={r.key} className="flex items-center gap-1.5">
                      ...
                      <span className="font-mono text-sm text-groove-vinyl font-medium">{total}</span>
                    </div>
                  )
                })}
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {myPosts.map(post => (
                <PostCard key={post.id} post={post} onAuthRequired={onAuthRequired} onReacted={loadArchive} />
              ))}
            </div>
          </div>
        )
        ...
        reactedPosts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="font-mono text-xs text-groove-label mt-2">Head to Rate Songs to start reacting.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {reactedPosts.map(post => {
              ...
              return (
                <div key={`${post.id}-${post.reacted_at}`} className="relative">
                  {/* My reaction badge */}
                  {myR && (
                    <div className="absolute -top-2 -right-2 z-10 bg-groove-vinyl text-groove-cream rounded-full w-8 h-8 flex items-center justify-center text-sm shadow border-2 border-groove-cream">
                      {myR.emoji}
                    </div>
                  )}
                  <PostCard post={post} onAuthRequired={onAuthRequired} onReacted={loadArchive} />
                  <p className="font-mono text-[10px] text-groove-dust mt-1 text-right px-1">
                    You reacted {formatDistanceToNow(new Date(post.reacted_at), { addSuffix: true })}
                  </p>
                </div>
              )
            })}
