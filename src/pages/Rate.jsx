...
export default function Rate({ onAuthRequired }) {
  ...
  async function loadQueue() {
    setLoading(true)
    try {
      // Get posts user hasn't reacted to
      const { data: reacted } = await supabase
        .from('reactions')
        .select('post_id')
        .eq('user_id', user.id)

      const reactedIds = reacted?.map(r => r.post_id) || []

      let query = supabase
        .from('posts')
        .select(`*, profiles:user_id (id, username, total_score)`)
        .neq('user_id', user.id) // don't rate your own
  ...
  if (!user) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        <div className="w-20 h-20 rounded-full bg-groove-vinyl mx-auto mb-6 flex items-center justify-center">
          <span className="text-3xl">🎵</span>
        </div>
        <h1 className="font-display font-black text-groove-vinyl text-3xl mb-3">Give Me Songs to Rate!</h1>
        <p className="font-mono text-xs text-groove-label mb-6">Sign in to start rating — your reactions help the community find the best picks.</p>
        <button
  ...
  if (done) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <p className="font-mono text-xs text-groove-label mb-2">
          You rated {sessionCount} song{sessionCount !== 1 ? 's' : ''} this session.
        </p>
        <p className="font-mono text-xs text-groove-dust mb-6">Check back later for new picks from the community.</p>
        <button
          onClick={loadQueue}
          className="bg-groove-vinyl text-groove-cream font-mono text-xs tracking-widest uppercase px-6 py-3 rounded hover:bg-groove-brown transition-colors"
        >
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="h-1 bg-groove-paper rounded-full mb-6 overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-groove-vinyl to-groove-red transition-all duration-500 rounded-full"
      ...
      <div className="bg-groove-cream border border-groove-dust/60 rounded-xl p-6 shadow-lg mb-6">
        ...
        <div className="flex flex-col items-center mb-5">
          <div className="w-28 h-28 rounded-full bg-groove-vinyl flex items-center justify-center shadow-2xl mb-4">
            {post.cover_url ? (
              ...
              <div className="w-10 h-10 rounded-full bg-groove-label flex items-center justify-center">
                <div className="w-3 h-3 rounded-full bg-groove-vinyl" />
              </div>
            )}
          </div>
        ...
        {post.note && (
          <p className="font-body text-sm text-groove-brown italic text-center border-t border-groove-dust/50 pt-4 leading-relaxed">
            "{post.note}"
          </p>
        )}

        <p className="font-mono text-[10px] text-groove-dust text-center mt-3">
          — recommended by {post.profiles?.username}
        </p>
      </div>
      ...
      <p className="font-mono text-[10px] text-groove-dust text-center mt-4">
        Your reactions earn the recommender points toward their rank.
      </p>
    </div>
  )
}
