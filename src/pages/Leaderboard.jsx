...
export default function Leaderboard() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {loading ? (
        <div className="space-y-2">{[...Array(5)].map((_, i) => <div key={i} className="h-14 bg-groove-paper rounded-lg animate-pulse" />)}</div>
      ) : users.length === 0 ? (
        ...
        <div className="space-y-2">
          {users.map((u, idx) => {
            const rank = getRank(u.total_score || 0)
            return (
              <Link key={u.id} to={`/profile/${u.id}`}
                ...
                <div className="w-8 text-center">
                  {idx < 3 ? <span className="text-lg">{medals[idx]}</span> : <span className="font-mono text-xs text-groove-dust">{idx + 1}</span>}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {u.streak > 2 && <span className="font-mono text-[10px] text-groove-amber">🔥{u.streak}d</span>}
                  </div>
